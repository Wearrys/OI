...
int H[maxn];
int Trans[20][maxn];
double dp[20][maxn], s[maxn];

inline double K(const pid& a, const pid& b) {
    return double(a.snd - b.snd) / (a.fst - b.fst);
}
...
int main() {
    ...
    for(int i = 1; i <= lim; i++) {
        ...
        for(int j = 2; j <= n; j++) {
            ...
            tmp = pid(j-2, s[j-1] - dp[i-1][j-1]);
            while(tail - head > 1 && K(Q[tail-1], tmp) < K(Q[tail-2], Q[tail-1])) --tail;
            Q[tail++] = tmp;

            tmp = pid(j, s[j]);
            while(tail - head > 1 && K(Q[head], tmp) < K(Q[head+1], tmp)) ++head;

            dp[i][j] = K(Q[head], tmp);
            Trans[i][j] = Q[head].fst + 1;
        }
    }
    static int Ed[maxn]; 
    int pos = Ed[lim] = n - (k - lim);

    for(int i = lim; i > 0; i--) Ed[i-1] = Trans[i][Ed[i]];

    Decimal ans = H[1];
    for(int i = 1; i <= lim; i++) {
        ans = (ans + s[Ed[i]] - s[Ed[i-1]]) / (Ed[i] - Ed[i-1] + 1);
    }
